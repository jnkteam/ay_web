...
$.formValidator = 
{
	...
	setTip : function(el,showclass,showmsg){
		var tip = $("#"+el);
		try{
			if(tip){
				tip.removeClass();
		...
		{		
			...
			if(setting.validatetype=="AjaxValidator")
			{
				...
				else
				{
					...
					/**jin add**/
					if(setting.onerrorResult()!='0'){
						showmsg=setting.onerrorResult();
					}
				}
			}
			else
	...
	showAjaxMessage : function(returnObj)
	{
		...
		else
		{
			if(setting.isvalid!=undefined && !setting.isvalid){
				...
				/**jin add**/
				if(setting.onerrorResult()!='0'){
					elem.lastshowmsg = setting.onerrorResult();
				}
			}
	...
	oneIsValid : function (id,index)
	{
		...
		{   
			...
			if(!settings[i].isvalid) {
				...
				returnObj.setting = settings[i];
				var handler = returnObj.setting.onerror;
				/**
				 * modify by jerry
				 * 如果验证失败，并且配置的验证对象可以跳过，则继续往下作验证
				 */
				if(returnObj.setting.skip){
					$.formValidator.setTipState(elem,"onError",handler)
				}else{
					break;
				}
			}else{
				returnObj.isvalid = true;
				returnObj.setting = settings[i].oncorrect?settings[i]:settings[0];
				var handler = returnObj.setting.oncorrect;
				
				/**
				 * modify by jerry
				 * 如果验证成功，并且处理器是函数，则执行该函数
				 */
				if(typeof(handler)=='function'){
					$.formValidator.setTipState(elem," ",handler)
				}
	...
	pageIsValid : function (validatorgroup)
	{
		...
		else
		{
			...
			if(thefirstid!="" && initConfig.errorfocus){$("#"+thefirstid).next().focus();}
		}
		...
		{	
			...
			success : function(data){
			    ...
			    else
			    {
			        ...
			        /**jin add**/
					if(setting.onerrorResult()!='0'){
						 $.formValidator.setTipState(elem,"onError",setting.onerrorResult());
					}
...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		else if (srcTag == "select")
		{
			...
			//失去焦点
			jqobj.bind("blur",function(){/*jqobj.trigger("change");*/$.formValidator.setTipState(this,"onShow",setting.onshow);});
			//选择项目后触发
...
$.fn.ajaxValidator = function(controlOptions)
{
	var settings = 
	{
		...
		buttons : null,
		onerror:"服务器校验没有通过",
		onerrorResult:function(){return "0";},
		onwait:"正在等待服务器返回数据",
