    ...
    $("#form_postorder button.green").click(function() {
        ...
        switch (channeltype) {
            case '107':
                ...
                break; //腾讯卡
            case '104':
                if (xkcardid.length != 15 && xkcardpass.length != 16) { $.dialog({ title: lktitle, content: channelname + cmgs + "<br />卡号为15位", lock: true, fixed: true, cancelVal: '确定', cancel: function() { $("#xk_cardId").focus(); }, icon: 'warning', width: '250px', height: '90px' }); return false; }
                else
