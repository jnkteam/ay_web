...
jQuery(document).ready(
	function() {
	    ...
	    if ($('.reg-form').length > 0) {
	        if ($('.reg-form input:eq(0)').val().length > 0) {
	            jQuery('.reg-form input:eq(0)').css({ color: "#333" });
	            jQuery("#rnametxt").css({ display: "none" });
	        }
	        if ($('.reg-form input:eq(1)').val().length > 0) {
	            jQuery('.reg-form input:eq(1)').css({ color: "#333" });
	            jQuery("#rpasstxt").css({ display: "none" });
	        }
	        if ($('.reg-form input:eq(2)').val().length > 0) {
	            jQuery('.reg-form input:eq(2)').css({ color: "#333" });
	            jQuery("#rpass2txt").css({ display: "none" });
	        }
	        if ($('.reg-form input:eq(3)').val().length > 0) {
	            jQuery('.reg-form input:eq(3)').css({ color: "#333" });
	            jQuery("#rlianxitxt").css({ display: "none" });
	        }
	        if ($('.reg-form input:eq(4)').val().length > 0) {
	            jQuery('.reg-form input:eq(4)').css({ color: "#333" });
	            jQuery("#rqqtxt").css({ display: "none" });
	        }
	        if ($('.reg-form input:eq(5)').val().length > 0) {
	            jQuery('.reg-form input:eq(5)').css({ color: "#333" });
	            jQuery("#rmailtxt").css({ display: "none" });
	        }
	    }
	    $("input.zc").click(function() { window.open("register.aspx") });
	    $("a.qqlogin").click(function() { window.open("/apps/qq.aspx") });
	    $("a.weibo").click(function() { window.open("/apps/weibo.aspx") });
	    $("#nametxt").click(function() { $("#nametxt").css({ display: "none" }); $("[name$='txtloginName']").focus(); /*document.aspnetForm.loginName.focus();*/ });
	    $("#passtxt").click(function() { $("#passtxt").css({ display: "none" }); $("[name$='txtloginPass']").focus(); /*document.aspnetForm.loginPass.focus();*/ });
	    $("#codetxt").click(function() { $("#codetxt").css({ display: "none" }); $("[name$='txtcheckCode']").focus(); /*document.aspnetForm.checkCode.focus();*/ });
	    $("#rnametxt").click(function() { $("#rnametxt").css({ display: "none" }); $("#login_name").focus(); /*document.reg.login_name.focus();*/ });
	    $("#rpasstxt").click(function() { $("#rpasstxt").css({ display: "none" }); $("#passwd1").focus(); /*document.reg.passwd1.focus();*/ });
	    $("#rpass2txt").click(function() { $("#rpass2txt").css({ display: "none" }); $("#passwd2").focus(); /*document.reg.passwd2.focus(); */ });
	    $("#rlianxitxt").click(function() { $("#rlianxitxt").css({ display: "none" }); $("#full_name").focus(); /*document.reg.full_name.focus();*/ });
	    $("#rqqtxt").click(function() { $("#rqqtxt").css({ display: "none" }); $("#qq").focus(); /*document.reg.qq.focus(); */ });
	    $("#rmailtxt").click(function() { $("#rmailtxt").css({ display: "none" }); $("#email").focus(); /*document.reg.email.focus();*/ });
	    $(".submit").click(function() {
	        if ($("#ddh").val().length < 18) {
	            alert(msg1); return false;
	        } 
	    });
	    $('#ddh').bind('onkeydown', function() {
